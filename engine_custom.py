...
from util import *
# from demo_custom import iou_score
...
class Engine(object):
    ...
    def validate(self, data_loader, model, criterion):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            self.state['iou_score'] = iou_score(self.state['target'],self.state['output'])
            # measure accuracy
            self.on_end_batch(False, model, criterion, data_loader)

        score1 = self.on_end_epoch(False, model, criterion, data_loader)

        return self.state['iou_score']
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if display:
            if training:
                ...
                print('Test: \t Loss {loss:.4f}\t mAP {map:.3f}\t iou_score {iou:.3f}'.format(loss=loss, map = map, iou=self.state['iou_score']))
                # print('OP: {OP:.4f}\t'
                #       'OR: {OR:.4f}\t'
                #       'OF1: {OF1:.4f}\t'
                #       'CP: {CP:.4f}\t'
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        feature_var = torch.autograd.Variable(self.state['feature']).float().cuda()
        target_var = torch.autograd.Variable(self.state['target']).float().cuda()
        inp_var = torch.autograd.Variable(self.state['input']).float().detach().cuda()  # one hot
        # if not training:
